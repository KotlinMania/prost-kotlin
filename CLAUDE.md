# Claude Code Project Instructions — prost-kotlin

## Project Overview

This is **prost-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`prost`](https://crates.io/crates/prost).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/prost/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/prost>
