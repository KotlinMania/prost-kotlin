...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Prost.swiftmodule/`
...
//      `import Prost` above would fail at compile time.
//
//   2. The static archive `libProst.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Prost" }` and
//      `flattenPackage = "io.github.kotlinmania.prost"` configuration in
...
final class ProstExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Prost swift module imported cleanly")
    }
}
